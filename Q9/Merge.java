 ...
 * Matricula: 748190
 * TP03 - Questão 9 - Ordenação por Mergesort em Java
 * 
****************************************/
...
class Mergesort {
    ...
    public void intercalar(int esq, int meio, int dir) {
        ...
        for (i = 0; i < n1; i++) {
            a1[i] = filme[esq + i].clone();
        }
        // -------------------------------------

        // --- inicializar segundo subarray ---
        for (j = 0; j < n2; j++) {
            a2[j] = filme[meio + 1 + j].clone();
        }
        // -------------------------------------
        ...
        while (i < n1 && j < n2) {
            if (a1[i].getOrcamento() <= a2[j].getOrcamento()) {
                filme[k] = a1[i].clone();
                i++;
            } else {
                filme[k] = a2[j].clone();
...
class Filme {
    ...
    private String folder = "./tmp/filmes/";
    ...
    public Filme clone() {
        ...
        clone.Nome = getNome();
        clone.Titulo_Original = getTitulo_Original();
        clone.Data_de_Lancamento = getData_de_Lancamento();
        clone.Genero = getGenero();
        clone.Idioma_Original = getIdioma_Original();
        clone.Situacao = getSituacao();
        clone.Orcamento = getOrcamento();
        clone.Key_Words = getKey_Words();
        clone.Duracao = getDuracao();
...
public class Merge {
    ...
    public static void main(String args[]) throws Exception {
        ...
        Mergesort movie = new Mergesort(10000); // criar uma lista da classe List
        ...
        Arq.openWrite("748190_mergesort.txt");
