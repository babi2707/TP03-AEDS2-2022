 ...
 * Matricula: 748190
 * TP03 - Questão 15 - Pilha Flexível em C
 * 
****************************************/
...
typedef struct Celula
{
    ...
    struct Celula *prox; // ponteiro que aponta para celula prox
} Celula;
...
int main() {
    ...
    int n = 0; // numero de linhas 2° parte do pub.in
    scanf("%i", &n);

    // ----- for para percorrer a 2° parte -----
    ...
    for (int i = 0; i < n; i++) {
        ...
        scanf(" %[^\n]", linha); // ler linha
        strcpy(nome, substring(linha, 3, strlen(linha)));
        
        // --- inserção ---
        if (linha[0] == 'I') {
            movie_inicio(&filme);
            movie_readHtml(&filme, nome);
            insert(filme);
        }
        // ----------------
